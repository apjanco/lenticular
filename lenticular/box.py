...
def get_box_client():
    ...
    if BOX_CLIENT_ID and BOX_CLIENT_SECRET and BOX_DEVELOPER_TOKEN:
        ...
        try:
            ...
            print('[orange_red1]Developer token is only valid for 60 minutes. Please get a new one at https://upenn.app.box.com/developers/console[/orange_red1]')
    else:
    ...
    def folder_contents(self, folder_id:str):
        ...
        for item in folder:
            if item.type == 'folder':
                ...
                contents.append({'name':item.name, 'id':item.id, 'download_url':item.get_download_url(),'path':file_path})
        return contents
